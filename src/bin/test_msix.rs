use installer_analyzer::analyzers::{AnalyzerFactory, InstallerAnalyzer, MsixAnalyzer};
use std::path::Path;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (file_path, expected_msix) in &test_files {
        ...
        if path.exists() {
            match analyzer.can_analyze(path).await {
                Ok(can_analyze) => {
                    let status = if can_analyze == *expected_msix {
                        "✓"
                    } else {
                        "✗"
                    };
        ...
        if path.exists() {
            match AnalyzerFactory::create_analyzer(path).await {
                Ok(factory_analyzer) => {
                    ...
                    let status = if is_msix == *expected_msix {
                        "✓"
                    } else {
                        "✗"
                    };
                    println!(
                        "  {} {}: Factory selected {:?} (msix = {})",
                        status,
                        file_path,
                        factory_analyzer.format(),
                        is_msix
    ...
    for (filename, expected) in &extension_tests {
        ...
        let has_msix_ext = path
            .extension()
            .and_then(|ext| ext.to_str())
            .map(|ext| matches!(ext.to_lowercase().as_str(), "msix" | "appx"))
            .unwrap_or(false);

        let status = if has_msix_ext == *expected {
            "✓"
        } else {
            "✗"
        };
        println!(
            "  {} {}: Extension check = {} (expected {})",
...
<Package xmlns="http://schemas.microsoft.com/appx/manifest/foundation/windows10">
  ...
  <Applications>
    ...
    let msix_supported =
        supported_formats.contains(&installer_analyzer::core::InstallerFormat::MSIX);
