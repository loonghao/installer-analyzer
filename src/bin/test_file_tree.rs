use installer_analyzer::analyzers::{InstallerAnalyzer, WheelAnalyzer};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let template_data =
        installer_analyzer::reporting::templates::ReportTemplateData::from_analysis_result(
            &analysis_result,
        );
    ...
    println!(
        "  - Total files in tree: {}",
        template_data.file_tree.total_files
    );
    println!(
        "  - Total directories in tree: {}",
        template_data.file_tree.total_directories
    );
    ...
    for (i, node) in template_data.file_tree.nodes.iter().take(5).enumerate() {
        ...
        if i >= 4 && template_data.file_tree.nodes.len() > 5 {
            println!(
                "  ... and {} more root nodes",
                template_data.file_tree.nodes.len() - 5
            );
            break;
        }
    }
    ...
    match report_generator
        .generate_report(&analysis_result, ReportFormat::Html)
        .await
    {
        Ok(html_report) => {
            ...
            match report_generator
                .save_report(&analysis_result, ReportFormat::Html, Path::new(output_path))
                .await
            {
                Ok(_) => {
                    println!("✓ Test report saved to: {}", output_path);
                    println!(
                        "  You can open this file in a browser to test the interactive file tree!"
                    );
                }
                Err(e) => {
    ...
    for (filename, expected_type) in &test_files {
        ...
        let status = if actual_type == *expected_type {
            "✓"
        } else {
            "❌"
        };
        println!(
            "  {} {}: {} (expected {})",
            status, filename, actual_type, expected_type
        );
    }
...
fn print_tree_node(node: &installer_analyzer::reporting::templates::FileTreeNode, depth: usize) {
    ...
    println!(
        "{}{}{}  {} ({})",
        indent, icon, node.name, node.file_type, node.size_formatted
    );
