...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if path.exists() {
            match AnalyzerFactory::create_analyzer(path).await {
                Ok(analyzer) => {
                    ...
                    let status = if detected_format == *expected_format {
                        "✓"
                    } else {
                        "✗"
                    };
    ...
    let unsupported_files = ["Cargo.toml", "README.md"];
        ...
        match AnalyzerFactory::create_analyzer(path).await {
            Ok(analyzer) => {
                ...
                match analyzer.extract_registry_operations(path).await {
                    Ok(operations) => {
                        println!(
                            "    ✓ Registry operations extracted: {} operations",
                            operations.len()
                        );
                    }
                    Err(e) => {
