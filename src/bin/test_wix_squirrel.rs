use std::path::Path;
use installer_analyzer::analyzers::{WixAnalyzer, SquirrelAnalyzer, InstallerAnalyzer, AnalyzerFactory};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_files = [
        ("tests/data/persistent_ssh_agent-0.9.0-py3-none-any.whl", false, false),  // Should not be WiX or Squirrel
    ...
    for (file_path, expected_wix, _expected_squirrel) in &test_files {
        ...
        if path.exists() {
            match wix_analyzer.can_analyze(path).await {
                Ok(can_analyze) => {
                    let status = if can_analyze == *expected_wix { "✓" } else { "✗" };
                    println!("  {} {}: WiX = {} (expected {})", 
    ...
    for (file_path, _expected_wix, expected_squirrel) in &test_files {
        ...
        if path.exists() {
            match squirrel_analyzer.can_analyze(path).await {
                Ok(can_analyze) => {
                    let status = if can_analyze == *expected_squirrel { "✓" } else { "✗" };
                    println!("  {} {}: Squirrel = {} (expected {})", 
    ...
    for (file_path, expected_wix, expected_squirrel) in &test_files {
        ...
        if path.exists() {
            match AnalyzerFactory::create_analyzer(path).await {
                Ok(factory_analyzer) => {
                    let is_wix = matches!(factory_analyzer.format(), installer_analyzer::core::InstallerFormat::WiX);
                    let is_squirrel = matches!(factory_analyzer.format(), installer_analyzer::core::InstallerFormat::Squirrel);
                    let wix_status = if is_wix == *expected_wix { "✓" } else { "✗" };
                    let squirrel_status = if is_squirrel == *expected_squirrel { "✓" } else { "✗" };
                    println!("  {} {} {}: Factory selected {:?} (wix={}, squirrel={})", 
                        wix_status, squirrel_status, file_path, factory_analyzer.format(), is_wix, is_squirrel);
                }
                Err(e) => {
    ...
    let wix_patterns = [
        ...
        "WiX v3", "WiX v4", "WiX v5",
        "wix.exe", "candle.exe", "light.exe",
        "WixUI", "WixUIExtension", "WixUtilExtension",
    ];
    ...
    let squirrel_patterns = [
        "Squirrel", "electron-builder", "electron-updater",
        "Update.exe", "SquirrelSetup", "app-update.yml",
        "latest.yml", "RELEASES", "nupkg", "Electron",
        "electron.exe", "resources\\app.asar", "autoUpdater",
    ];
    ...
    let squirrel_supported = supported_formats.contains(&installer_analyzer::core::InstallerFormat::Squirrel);
    println!("  ✓ WiX format in supported list: {}", wix_supported);
    println!("  ✓ Squirrel format in supported list: {}", squirrel_supported);
    ...
    if let Some(wix_analyzer_by_format) = installer_analyzer::analyzers::AnalyzerFactory::get_analyzer_by_format(
    ...
    if let Some(squirrel_analyzer_by_format) = installer_analyzer::analyzers::AnalyzerFactory::get_analyzer_by_format(
    ...
    for file_path in &msi_test_files {
        ...
        if path.exists() {
            ...
            if is_msi {
                ...
                let matches = installer_analyzer::analyzers::common::search_file_content(path, &wix_patterns).await?;
                println!("    ✓ WiX pattern matches: {} (expected 0 for non-WiX MSI)", matches.len());
            }
        } else {
            println!("  - {}: File not found", file_path);
        }
    ...
    for file_path in &nsis_test_files {
        let path = Path::new(file_path);
        if path.exists() {
            println!("  Testing Squirrel delegation with NSIS file: {}", file_path);
            ...
            if is_nsis {
                ...
                let matches = installer_analyzer::analyzers::common::search_file_content(path, &squirrel_patterns).await?;
                println!("    ✓ Squirrel pattern matches: {} (expected 0 for non-Squirrel NSIS)", matches.len());
            }
        } else {
            println!("  - {}: File not found", file_path);
        }
