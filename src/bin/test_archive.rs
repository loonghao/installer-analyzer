use installer_analyzer::analyzers::{common, ArchiveAnalyzer, InstallerAnalyzer};
use std::path::Path;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (file_path, expected_archive) in &test_files {
        ...
        if path.exists() {
            match analyzer.can_analyze(path).await {
                Ok(can_analyze) => {
                    let status = if can_analyze == *expected_archive {
                        "✓"
                    } else {
                        "✗"
                    };
    ...
    let archive_files = ["tests/data/persistent_ssh_agent-0.9.0-py3-none-any.whl"];
