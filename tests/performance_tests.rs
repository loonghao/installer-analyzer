...
async fn benchmark_analysis_performance() {
    let test_files = vec![
        ...
        ("persistent_ssh_agent-0.9.0-py3-none-any.whl", "Python Wheel"),
        ("ShareX-17.1.0-portable.zip", "ZIP Archive"),
    ];
    
    println!("\n=== Performance Benchmark Results ===");
    println!("{:<40} {:<15} {:<15} {:<10}", "File", "Size (MB)", "Time (ms)", "Rate (MB/s)");
    println!("{}", "-".repeat(80));
...
async fn test_memory_usage_with_large_files() {
    ...
    for filename in large_files {
        ...
        println!("Testing memory usage for {} ({:.2} MB)", filename, file_size_mb);
        ...
        match result {
            Ok(_) => {
                ...
                if output_file.exists() {
                    ...
                    assert!(output_size < metadata.len() * 2, 
...
async fn test_concurrent_analysis() {
    ...
    println!("Testing concurrent analysis with {} files", available_files.len());
    ...
    for (i, (filename, file_path)) in available_files.into_iter().enumerate() {
        let output_file = temp_dir.path().join(format!("concurrent_output_{}.json", i));
        let file_path_clone = file_path.clone();
        
        let task = tokio::spawn(async move {
            let start = Instant::now();
            let result = handle_analyze(&file_path_clone, Some(&output_file), Some("json"), false).await;
            let duration = start.elapsed();
    ...
    for result in results {
        match result {
            Ok((filename, analysis_result, duration)) => {
    ...
    assert!(successful > 0 || failed > 0, "At least some analyses should complete");
}
...
async fn test_format_conversion_performance() {
    ...
    for (format, format_name) in formats {
        let output_file = temp_dir.path().join(format!("format_test.{}", 
...
async fn test_stress_analysis_repeated() {
    ...
    if !durations.is_empty() {
        ...
        assert!(variance < 3.0, "Performance variance should be reasonable (got {:.2}x)", variance);
    }
}
