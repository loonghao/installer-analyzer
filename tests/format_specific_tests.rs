    ...
    match result {
        Ok(Ok(_)) => {
            ...
            let json_value: serde_json::Value =
                serde_json::from_str(&content).expect("MSI analysis should produce valid JSON");
    ...
    for (filename, _expected_name) in exe_files {
        ...
        match result {
            Ok(_) => {
                assert!(
                    output_file.exists(),
                    "EXE analysis should create output for {}",
                    filename
                );

                let content = std::fs::read_to_string(&output_file).unwrap();
                let json_value: serde_json::Value =
                    serde_json::from_str(&content).expect("EXE analysis should produce valid JSON");

                // Check for basic structure
                assert!(
                    json_value.get("metadata").is_some(),
                    "EXE analysis should have metadata for {}",
                    filename
                );
            ...
            Err(e) => {
                println!(
                    "EXE analysis failed for {} (may be expected): {}",
                    filename, e
                );
            }
        }
...
async fn test_python_wheel_analysis() {
    ...
    match result {
        Ok(_) => {
            ...
            let content = std::fs::read_to_string(&output_file).unwrap();
            let json_value: serde_json::Value =
                serde_json::from_str(&content).expect("Wheel analysis should produce valid JSON");
            ...
            if let Some(metadata) = json_value.get("metadata") {
                ...
                if let Some(version) = metadata.get("version") {
                    assert!(
                        version.as_str().unwrap().contains("0.9.0"),
            ...
            if let Some(files) = json_value.get("files") {
                ...
                let has_python_files = file_names
                    .iter()
...
async fn test_zip_archive_analysis() {
    ...
    match result {
        Ok(_) => {
            ...
            let content = std::fs::read_to_string(&output_file).unwrap();
            let json_value: serde_json::Value =
                serde_json::from_str(&content).expect("ZIP analysis should produce valid JSON");
            ...
            if let Some(files) = json_value.get("files") {
                ...
                let exe_files: Vec<&str> = files_array
                    .iter()
                    .filter_map(|f| f.get("name"))
                    .filter_map(|n| n.as_str())
                ...
                if !exe_files.is_empty() {
                    ...
                    for exe_file in exe_files.iter().take(5) {
                        // Show first 5
                        println!("  - {}", exe_file);
                    }
                }
...
async fn test_output_format_consistency() {
    ...
    for (format, _expected_type) in formats {
        ...
        let output_file = temp_dir
            .path()
            .join(format!("consistency_test.{}", extension));
        ...
        match result {
            Ok(_) => {
                assert!(
                    output_file.exists(),
                    "Output file should exist for {} format",
                    format
                );
