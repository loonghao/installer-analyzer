...
mod tests {
    ...
    async fn test_analyzer_factory_with_real_files() {
        let test_files = vec![
            ...
            (
                ...
                Some(InstallerFormat::PythonWheel),
            ),
            (
                "tests/data/Gitify.Setup.6.3.0.exe",
                Some(InstallerFormat::NSIS),
            ),
        ];
            ...
            match AnalyzerFactory::create_analyzer(path).await {
                Ok(analyzer) => {
                    if let Some(expected) = expected_format {
                        ...
                        assert!(
                            can_analyze,
                            "Analyzer should be able to analyze {}",
                            file_path
                        );
                ...
                Err(e) => {
                    if expected_format.is_some() {
                        panic!(
                            "Expected to create analyzer for {} but got error: {}",
                            file_path, e
                        );
                    } else {
    ...
    async fn test_analyzer_factory_unsupported_files() {
        let unsupported_files = vec!["Cargo.toml", "README.md"];
        ...
        for file_path in unsupported_files {
            ...
            assert!(
                result.is_err(),
                "Should reject unsupported file: {}",
                file_path
            );
            println!("✓ Correctly rejected unsupported file: {}", file_path);
        }
    ...
    async fn test_get_analyzer_by_format() {
        ...
        for format in test_formats {
            ...
            assert!(
                analyzer.is_some(),
                "Should have analyzer for format: {:?}",
                format
            );
            ...
            assert_eq!(
                analyzer.format(),
                format,
                "Analyzer format should match requested format"
            );
        ...
        assert!(
            analyzer.is_none(),
            "Should not have analyzer for Unknown format"
        );
    }
