...
async fn test_analyze_real_exe_files() {
    ...
    for exe_filename in exe_files {
        ...
        let temp_dir = TempDir::new().unwrap();
        let output_file = temp_dir.path().join(format!("{}_analysis.html", 
        ...
        match result {
            Ok(_) => {
                assert!(output_file.exists(), "Output file should be created for {}", exe_filename);
                
                let content = std::fs::read_to_string(&output_file).unwrap();
                assert!(!content.is_empty(), "Output file should not be empty for {}", exe_filename);
                
                // Check for HTML structure
                assert!(content.contains("<html>") || content.contains("<!DOCTYPE"), 
                    "Should contain HTML structure for {}", exe_filename);
            }
            Err(e) => {
                println!("Analysis failed for {} (may be expected): {}", exe_filename, e);
            }
        }
    }
}
        ...
        match result {
            Ok(_) => {
                assert!(output_file.exists(), 
                    "Output file should be created for {} -> {}", input_filename, output_filename);
                
                let content = std::fs::read_to_string(&output_file).unwrap();
                assert!(!content.is_empty(), 
                    "Output file should not be empty for {} -> {}", input_filename, output_filename);
            ...
            Err(e) => {
                println!("Analysis failed for {} (may be expected): {}", input_filename, e);
            }
        }
    }
}
...
async fn test_sandbox_with_real_exe_file() {
    ...
    if !exe_file.exists() {
        println!("Skipping sandbox test: EXE file not found at {:?}", exe_file);
        return;
    }
    ...
    match result {
        Ok(_) => {
            assert!(output_file.exists(), "Sandbox output file should be created");
        ...
        Err(e) => {
            ...
            println!("Sandbox analysis failed (may be expected in test environment): {}", e);
        }
    }
}
...
async fn test_file_type_detection_accuracy() {
    ...
    for (filename, expected_type) in test_files {
        ...
        assert_eq!(extension.to_lowercase(), expected_type,
            "File {} should have extension {}", filename, expected_type);
        ...
        println!("✓ {} ({} bytes) - type: {}", filename, metadata.len(), expected_type);
    }
}
...
async fn test_error_handling_with_corrupted_files() {
    ...
    match result {
        Ok(_) => {
            ...
            assert!(output_file.exists(), "Should create output file even for fake installer");
        }
        Err(e) => {
...
async fn test_large_file_handling() {
    ...
    match result {
        Ok(_) => {
            ...
            let _: serde_json::Value = serde_json::from_str(&content)
                .expect("Large file output should be valid JSON");
        }
        Err(e) => {
